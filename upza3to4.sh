...
cd /usr/ports/net-mgmt/zabbix4-proxy && make -ss config-recursive && make -ss install clean
if ( $? == 0 ) then
	echo "Sem falha na instalacao do proxy, continuando"
	pkg lock -y zabbix4-proxy
	rehash
else if ( $? > 0 ) then
...
cd /usr/ports/net-mgmt/zabbix4-agent && make -ss config-recursive && make -ss install clean
if ( $? == 0 ) then 
        echo "Sem falha na instalacao do agente, continuando"
	pkg lock -y zabbix4-agent
	rehash
else if ( $? > 0 ) then
...
if ( -d /usr/local/etc/zabbix34 ) cd /usr/local/etc/zabbix34 && mv * ../zabbix4
